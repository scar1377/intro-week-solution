...
skipTest("returns a single pair in an array when passed one", () => {
  check(pairDNA).whenCalledWith("G").returns(["GC"]);
});
skipTest("returns multiple pairs when passed multiple chars", () => {
  check(pairDNA).whenCalledWith("GGC").returns(["GC", "GC", "CG"]);
});
skipTest("returns correctly when passed lowercase letters", () => {
  check(pairDNA).whenCalledWith("gat").returns(["GC", "AT", "TA"]);
});
skipTest("ignores invalid chars but still returns valid ones within the string", () => {
