...
runTest("returns unique ordered numbers from an array", function () {
  check(uniqueAndOrdered([1, 1, 1, 1, 1, 2, 2, 2, 3, 3, 1, 1])).isEqualTo([1, 2, 3, 1]);
});

skipTest("returns unique ordered letters from a string", function () {
  check(uniqueAndOrdered("nnoorrtthhccooddeerrss")).isEqualTo(["n", "o", "r", "t", "h", "c", "o", "d", "e", "r", "s"]);
});

skipTest("is case sensitive for strings", function () {
  check(uniqueAndOrdered("AaAAABBBCCCc")).isEqualTo(["A", "a", "A", "B", "C", "c"]);
});
